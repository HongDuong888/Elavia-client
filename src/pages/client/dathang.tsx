...
const Dathang = () => {
  ...
  const [showAddressModal, setShowAddressModal] = useState(false); // Thêm dòng này
  const [selectedAddress, setSelectedAddress] = useState(null); // Thêm dòng này,
  ...
  // Đặt các biến kiểm soát hiển thị
  let content = null;
  ...
  // Thêm useEffect để khởi tạo selectedAddress khi userData có giá trị
  useEffect(() => {
    if (userData && userData.length > 0) {
      setSelectedAddress(userData[0]); // Cập nhật selectedAddress với địa chỉ đầu tiên
    }
  }, [userData]); // Thêm dòng này

  // Thay các chỗ sử dụng userData[0] thành selectedAddress (nếu có)
  const currentAddress = selectedAddress || (userData && userData.length > 0 ? userData[0] : null);
  ...
  const handlePayment = async () => {
    ...
    // Sử dụng selectedAddress nếu có, nếu không thì dùng userData[0]
    const currentAddress = selectedAddress || userData[0]; // Thêm dòng này
    const addressStr =
      currentAddress.address +
      ", " +
      currentAddress.district.name +
      ...
      currentAddress.commune.name +
      ", " +
      currentAddress.city.name; // Thay thế address bằng addressStr với currentAddress
      ...
      if (paymentMethod === "cod") {
        const payload = {
          ...
          user: {
            ...
            address: addressStr, // Sử dụng addressStr
          },
        ...
        <ClientLayout>
          <article className="mt-[100px]">
            <article className="grid grid-cols-[4fr_1.5fr] gap-10 mt-[100px]">
              <div>
                <div className="grid grid-cols-[1fr] gap-10">
                  <div>
